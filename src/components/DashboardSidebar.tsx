...
import {
  Home, BookOpen, Upload, Settings, LogOut, Users, ClipboardList, UserPlus, BookCheck, UserCircle, UsersRound, ChevronsLeft, ChevronsRight
} from "lucide-react";
      ...
      <div className="p-2 border-t border-white/10">
        <div className={cn("flex items-center p-2", !isExpanded && "justify-center")}>
          ...
          <div className={cn("ml-3 overflow-hidden transition-all duration-200", !isExpanded && "w-0")}>
            ...
            <div className="text-xs text-gray-300 truncate">{profile?.email || "user@example.com"}</div>
          </div>
        </div>
        <Tooltip>
          <TooltipTrigger asChild>
            <button className={cn(
              'w-full flex items-center h-10 px-3 rounded-lg text-white/80 transition-colors hover:bg-white/10',
              isExpanded ? "justify-start gap-3" : "justify-center"
            )}>
